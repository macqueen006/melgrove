...
const Cases = () => {
    return (
        <section className="cases relative block w-full h-full">
            <div className="container-fluid w-full h-full relative">
                <div
                    className="cases-navigation absolute bottom-0 w-full hidden sm:flex justify-between items-center z-10 px-8 pb-6">
                    <div
                        className="cases-arrow prev disabled bg-slate-900/70 h-12 w-12 rounded-full grid place-content-center">
                        <CaseLeft className="h-6 w-6 stroke-white/50"/>
                    </div>
                    <div className="cases-arrow bg-slate-900/70 h-12 w-12 rounded-full grid place-content-center next">
                        <CaseRight className="h-6 w-6 stroke-white"/>
                    </div>
                </div>
                <div className="row flex flex-col border-2 sm:flex-row w-full h-full z-[1]">
                    {products.map((item) => (
                        <div
                            key={item.id}
                        ...
                        >
                            <div
                                className="case-details w-full h-[50vh] text-white flex flex-col justify-center p-4 sm:p-8 relative z-[3]">
                                <span className="text-sm mt-[156px] sm:mt-[90px]">{item.subTitle}</span>
                                <h2 className="font-bold mt-2 text-lg w-5/7 sm:w-6/7">{item.title}</h2>
                            </div>
                            <div
                                className="case-img w-full h-full absolute opacity-60 group-hover:opacity-40 transition-all duration-300 top-0 left-0 overflow-hidden z-[2]">
                                <Image
                                    id="product-img"
                                    layout="fill"
                                    sizes="100%"
